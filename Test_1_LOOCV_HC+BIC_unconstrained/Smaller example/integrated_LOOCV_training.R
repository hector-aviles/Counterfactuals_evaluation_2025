...
dt <- subset(dt, select = c(action, free_NE, latent_collision))
dt <- subset(dt, action == "cruise" | action == "keep" )
#dt <- dt[complete.cases(dt)]
dt_factor <- as.data.frame(lapply(dt, factor))
head(dt_factor)

# --- Blacklist construction (kept as original)
bl_a <- data.frame(from = c("action"), to = c("free_NE"))

bl_lcol <- data.frame(from = c("latent_collision", "latent_collision"), to = c("action",  "free_NE"))

bl <- rbind(bl_a, bl_lcol)

# --- Structural learning (hc)
network_structure <- hc(dt_factor, whitelist = NULL, blacklist = NULL, score = "bic", debug = FALSE, restart = 0, perturb = 1, max.iter = Inf, maxp = Inf, optimized = TRUE)
