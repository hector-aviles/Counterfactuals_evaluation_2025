...
for (r_idx in seq_along(reps)) {
  ...
  for (percentage in percentages) {
    ...
    for (i in 1:n_folds) {
      ...
      plot(net_mmpc, main = paste("MMPC Skeleton – Fold", i, "(alpha=0.05)"))
      dev.off()
    
      # === Skeleton diagnostics (pure base R – no bnlearn component functions) ===
      skeleton_edges <- nrow(arcs(net_mmpc))
      
      # Adjacency matrix → undirected graph connectivity via BFS
      adj <- amat(net_mmpc)                  # symmetric 0/1 matrix
      nodes <- rownames(adj)
      ...
      for (start in seq_len(n_nodes)) {
        if (!visited[start]) {
          ...
          while (length(queue) > 0) {
            ...
            if (!visited[u]) {
              visited[u] <- TRUE
              component <- c(component, u)
              neighbors <- which(adj[u, ] == 1)
              queue <- unique(c(queue, neighbors[!visited[neighbors]]))
            }
          }
          components <- c(components, list(nodes[component]))
        }
      }
      
      skeleton_components <- length(components)
      skeleton_connected <- (skeleton_components == 1)
      
      message("MMPC skeleton – Fold ", i, ": ",
              skeleton_edges, " undirected edges | ",
              "Connected: ", skeleton_connected, " | ",
              skeleton_components, " components")
      
      # === Blacklist from skeleton ===
      skel_mat <- adj
      
      missing <- which(skel_mat == 0 & row(skel_mat) != col(skel_mat), arr.ind = TRUE)
      
      blacklist_df <- data.frame(
        from = rownames(skel_mat)[missing[, "row"]],
        to   = colnames(skel_mat)[missing[, "col"]],
        stringsAsFactors = FALSE
      )
      blacklist_df <- unique(blacklist_df)
      
      message("Blacklisted directed arcs: ", nrow(blacklist_df))
      
      # Hill-climbing with blacklist
      network_structure <- hc(
        df_factor,
      ...
      message("Final network arcs after HC: ", final_arcs)
      
      end_time_struct <- Sys.time()
      elapsed_time_struct <- as.numeric(end_time_struct - start_time_struct, units = "secs")
      
      # === Save summary to file ===
      summary_file <- file.path(cbns_dir, "skeleton_summary.txt")
      
      summary_lines <- c(
        paste("Fold", i, "------------------------------"),
        paste("Date/Time:", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
        paste("MMPC Skeleton edges (undirected):", skeleton_edges),
        paste("Skeleton is connected:", skeleton_connected),
        paste("Number of connected components in skeleton:", skeleton_components),
        paste("Final oriented network arcs:", final_arcs),
        paste("Training time (structure):", round(elapsed_time_struct, 2), "seconds"),
        ""
      )
      
      cat(summary_lines, file = summary_file, sep = "\n", append = TRUE)
