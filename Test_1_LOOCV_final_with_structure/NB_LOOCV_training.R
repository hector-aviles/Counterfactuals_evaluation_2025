...
# Function to create and train NB model using Python with temporary files
train_nb_model <- function(train_sample, rep_num, percentage, fold_num, nb_dir) {
  # Create temporary file for training data
  temp_train_file <- tempfile(pattern = paste0("train_rep", rep_num, "_pct", percentage, "_fold", fold_num, "_"), 
                             fileext = ".csv")
  
  # Convert to data frame with only required columns and save to temp file
  train_df <- as.data.frame(train_sample)[, c("action", "curr_lane", "free_E", "free_NE", "free_NW", "free_SE", "free_SW", "free_W")]
  fwrite(train_df, temp_train_file)
  
  # Create model path
  model_path <- file.path(nb_dir, paste0("NB_fold_", fold_num, ".pkl"))
  
  # Call Python script with the temporary file
  python_script <- "create_NB.py"
  cmd <- sprintf("python3 %s --train_file %s --percentage %s --rep_num %d --fold_num %d --model_path %s",
                python_script, temp_train_file, percentage, rep_num, fold_num, model_path)
  ...
  for (line in system_result) {
    message(line)
  }
  
  # Clean up temporary file
  if (file.exists(temp_train_file)) {
    file.remove(temp_train_file)
    message("Temporary training file removed: ", temp_train_file)
  }
  
  return(list(
    training_time = training_time,
    model_path = model_path,
    python_output = system_result
  ))
}
...
for (r_idx in seq_along(reps)) {
  ...
  for (percentage in percentages) {
    ...
    for (i in 1:n_folds) {
      ...
      # Train model using temporary file approach
