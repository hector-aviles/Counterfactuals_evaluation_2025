...
def process_combo(combo_dir, no_crashes_df, frequency_dict):
    ...
    missing_folds  = [f for f in range(all_fold_ids[0], all_fold_ids[-1] + 1)
                      if f not in all_fold_ids] if all_fold_ids else []

    print(f"  Total rows in CSV        : {len(df)}")
    print(f"  Unique folds (group_ids) : {n_folds_found}")
    print(f"  Fold ID range            : {all_fold_ids[0]} – {all_fold_ids[-1]}")
    if missing_folds:
        print(f"  [Warning] Missing fold IDs (no rows at all): {missing_folds}")
    else:
        print(f"  All fold IDs present in range — no gaps.")
    if incomplete:
...
def main():
    ...
    if process_all:
        combo_dirs = sorted([
            ...
            if os.path.isdir(d) and d.startswith('hc_')
        ])
        if not combo_dirs:
            print("[Error] No hc_* directories found in current directory.")
            sys.exit(1)
