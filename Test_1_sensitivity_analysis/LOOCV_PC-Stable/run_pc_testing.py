...
#
# Usage:
#   python3 run_pc_testing.py <combo_dir> [--folds 1-78]
#
# Arguments:
...
def parse_args():
    args = sys.argv[1:]
    if not args:
        print("Usage: python3 run_pc_testing.py <combo_dir> [--folds START-END]")
        sys.exit(1)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  PC sensitivity testing: {combo_name}")
    ...
    if missing_from_agg:
        ...
        print(f"  Rerun: python3 run_pc_testing.py {combo_dir} "
    ...
    if failed:
        ...
        print(f"  Rerun with: python3 run_pc_testing.py {combo_dir} --folds <N>-<N>")
